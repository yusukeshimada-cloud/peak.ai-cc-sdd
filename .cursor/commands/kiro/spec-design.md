...
3. **Split design artifacts (this workspace)** — follow **`.cursor/rules/spec-design-split-design-files.mdc`** in addition to the template:
   - **Do not** generate `.kiro/specs/common/design.md` or a separate `common.md` for design.
   - Put prerequisites, cross-feature hub content, and feature summary in **`.kiro/specs/$1/design.md`** (merge with template content as needed; keep hub/summary and references to splits clear).
   - Under **`.kiro/specs/$1/design/`**, create only applicable split files: **`ui.md`**, **`api.md`**, **`batch.md`**, **`integration.md`**, **`report.md`** (see rule for the role of each).
   - If stubs are required, add **`.kiro/specs/$1/stub.md`**.
...
1. **Status**: Confirm design at `.kiro/specs/$1/design.md` and applicable splits under `.kiro/specs/$1/design/` (and `stub.md` if used), per workspace split rule
